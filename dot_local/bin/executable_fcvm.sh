...
DEB_REPO_CURRENT_PATH="${DEB_REPO_CURRENT_PATH:-apt-current-test}"
DEB_REPO_LEGACY_PATH="${DEB_REPO_LEGACY_PATH:-apt-legacy-test}"
...
echo "deb [arch=\$_A signed-by=/etc/apt/trusted.gpg.d/faircom-packages.gpg] ${REPO_BASE_URL}/${DEB_REPO_PATH} ${DEB_REPO_SUITE} main" | sudo tee /etc/apt/sources.list.d/faircom.list >/dev/null
